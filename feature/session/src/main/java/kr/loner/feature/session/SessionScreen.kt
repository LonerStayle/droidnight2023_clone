...
fun SessionScreen(
    ...
    val sessionState =
        (sessionUiState as? SessionUiState.SessionList)?.sessionList?.let { sessionList ->
            rememberSessionState(sessions = sessionList)
        } ?: rememberSessionState(sessions = persistentListOf())

    LaunchedEffect(true) {
        sessionViewModel.errorFlow.collectLatest { throwable ->
